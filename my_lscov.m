function [x,stdx,mse,S] = my_lscov(A,b,V,alg, maxit, tol, m_initial, cov_cols, NoDrop)
...
switch lower(alg)
    case 'chol'
        ...
        if NoDrop   % Sometimes the QR leaves zeros on the diagonal even though a parameter is reasonably well constrained.  Usually generates a warning message later on.
            keepCols(:)=true;
        end
        
        % keep everything if cov_cols is not defined
        if ~exist('cov_cols','var') || isempty(cov_cols); cov_cols=true(size(perm)); end
        ...
        if nargout > 1
            ...
            % my scheme for limiting the memory use of lscov
            R=R(cov_cols(perm)~=0,:); 
            R=R(:, cov_cols(perm)~=0);
            %           Rinv=R\eye(size(R,1), outClass);
            % include a scaling factor that gives the ratio between the
            % true errors and the nominal errors used in the inversion
            % formula (stored in cov_cols, assuming they are nonzero)
            %temp=cov_cols(perm);
            %temp=temp(temp~=0);
            %Rinv=R\spdiags(double(temp(:)), 0, size(R,1), size(R,1));
            Rinv=R\speye(size(R,1));
            perm=perm(cov_cols(perm)~=0);
            if nargout > 3
                if issparse(x)
                    S = sparse(nvar,nvar);
                else
                ...
                % avoid squaring the whole Rinv matrix, which takes lots of
                % memory
                blocksize=1000;
                blocks=1:blocksize:size(Rinv,1);
                SumRinv= zeros(size(Rinv,1),1);
                for k=1:length(blocks);
                    these=blocks(k):min(blocks(k)+blocksize, size(Rinv,1));
                    temp=Rinv(these,:);
                    SumRinv(these)=sum(temp.*conj(temp),2);
                end
                stdx(perm,1:nrhs) = sqrt(SumRinv*mse);
            end
        end
    ...
    case {'lsqr', 'lsmr'}
        ...
        if all(alg=='lsqr')
            if exist('m_initial','var');
                x=lsqr(A, b, tol, maxit, [], [], m_initial);
            else
                x=lsqr(A, b, tol, maxit);
            end
        else
            x=lsmr(A, b, [], tol, tol, 0,  maxit, 100, 0);%, m_initial);
        end
