function [dM, dCtr, syn_wf]=correct_for_TX_shape_bnds( t_TX, TX, HW, sigma_rx, SNR)
...
if exist('SNR', 'var')
    ...
    % add in noise values
    TX_WF_broadened=TX_WF_broadened/sum(TX_WF_broadened(els))+1/SNR/(HW/dt);
    
    % iterate to find the centroid
    last_t_ctr=t1(1);
    %count=0; 
    while abs(t_ctr-last_t_ctr) > 1e-5/1.5e8
        last_t_ctr=t_ctr;
        t_ctr=wf_centroid(t1, TX_WF_broadened, t_ctr+[-HW/2 HW/2]);
        %count=count+1;
...
TXBm=wf_percentile_bnds(t1, TX_WF_broadened, 0.5, TXBm0+[-HW/2 HW/2]);
 
%dM=TXm-RXm;
TimeToH=1.5e8;
dM=TXBm*TimeToH;
dCtr=wf_centroid(t1, TX_WF_broadened, t_ctr+[-HW/2 HW/2])*TimeToH;;
...
if 0
    ...
    %HW_vals=3+([0:0.05:1, 2:18]);
    HW_vals=3+([0:0.01:.5]);
    
    
    SNR_vals=logspace(0, 3, 4);
    
    % the minimum SNR is 10 PE /( 6 MHz * 20 m /1.5e8 m/s * 57 pulses) =
    % 0.22
    % 
    ...
    for kSNR=1:length(SNR_vals)
        for kWp=length(W_pulse):-1:1
            for kHW=1:length(HW_vals)
                [dMb(kWp,  kHW, kSNR), dCtrb(kWp, kHW, kSNR)]=correct_for_TX_shape_bnds(WF.t, WF.p, HW_vals(kHW)/1.5e8,  W_pulse(kWp)/1.5e8, SNR_vals(kSNR));
            end
        end
    end
