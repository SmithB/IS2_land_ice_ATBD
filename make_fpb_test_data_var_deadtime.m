...
test_data_dir=['/home/ben/temp/deadtime_correction_data/var_deadtime_0MHz'];
...
D=csvread('~/Dropbox/projects/IS2_ATBD/deadtime_by_channel/CAL_42_PRIM_2016298_a.csv', 17,0);
DT=reshape(D(:,3), [20, 3]); DTS=DT(1:16,:); DTW=DT(17:end,:);
for k=1:3
    if DOMEAN
        t_dead{2*k-1}=zeros(size(DTW(:,k)))+mean(DTW(:,k));
        t_dead{2*k}=zeros(size(DTS(:,k)))+mean(DTS(:,k));
    else
        t_dead{2*k-1}=DTW(:,k);
        t_dead{2*k}=DTS(:,k);
    end
 ...
 for k=1:numel(Rough)
    ...
    if exist(D2_file,'file')
        ...
        clear D2 params;
        for beam=1:2
            [D2(beam), params(beam)]=make_ATL03_data_vdt(N_pulses, N_chan(beam), Rough(k), N_per_white(beam)*Rsurf(k), WF, BGR, H_win, 0, t_dead{beam});
        end
    ...
    for k0=1:max(D2(2).segment_number)-1
        for kB=1:2
            ...
            [med, centroid, count, N_fpb_corr, sigma_med, sigma_centroid, minGain, gain_full]=...
                fpb_corr(t_WF_full, uncorrected_counts,  params(kB).N_channels, N_pulses_est, mean(t_dead{beam}), signal_threshold_for_gain_corr, delta_t);
 ...
 %-------------------------------------------------------
function [D2, params]=make_ATL03_data_vdt(N_pulses, N_chan, roughness, N_per_pulse, WF, BGR, H_window, AT_slope, t_dead)
...
params=struct('roughness', roughness,   'sigma_x', 7.5, ...
    ...
    'N_channels', N_chan, 'N_per_pulse', N_per_pulse, 't_dead', t_dead);
D2=det_sim_var_deadtime(DEM, x0, params, ATM_xmit);
...
fprintf(1,'\n\nmedian, uncorrected, weak\n');
fprintf(1,'\t%2.2f', Rsurf(1,:));
fprintf(1,'\n');
thestr=repmat('\t%2.2f', 1,size(C(1).med,2));
fprintf(1, ['%2.1f', thestr,'\n'], [Rough(:,1), U(1).med*1000]')


fprintf(1,'\n\nmedian, corrected, weak\n');
fprintf(1,'\t%2.2f', Rsurf(1,:));
fprintf(1,'\n');
thestr=repmat('\t%2.2f', 1,size(C(1).med,2));
fprintf(1, ['%2.1f', thestr,'\n'], [Rough(:,1), C(1).med*1000]')

fprintf(1,'\n\nmean, uncorrected, weak\n');
fprintf(1,'\t%2.2f', Rsurf(1,:));
fprintf(1,'\n');
thestr=repmat('\t%2.2f', 1,size(C(1).med,2));
fprintf(1, ['%2.1f', thestr,'\n'], [Rough(:,1), U(1).mean*1000]')

fprintf(1,'\n\nmean, corrected, weak\n');
fprintf(1,'\t%2.2f', Rsurf(1,:));
fprintf(1,'\n');
thestr=repmat('\t%2.2f', 1,size(C(1).med,2));
fprintf(1, ['%2.1f', thestr,'\n'], [Rough(:,1), C(1).mean*1000]')

  
fprintf(1,'\n\napparent reflectance, uncorrected, weak\n');
fprintf(1,'\t%2.2f', Rsurf(1,:));
fprintf(1,'\n');
thestr=repmat('\t%2.2f', 1,size(C(1).med,2));
fprintf(1, ['%2.1f', thestr,'\n'], [Rough(:,1), U(1).N./(3*58*Rsurf)]')

fprintf(1,'\n\napparent reflectance, corrected, weak\n');
fprintf(1,'\t%2.2f', Rsurf(1,:));
fprintf(1,'\n');
thestr=repmat('\t%2.2f', 1,size(C(1).med,2));
fprintf(1, ['%2.1f', thestr,'\n'], [Rough(:,1), C(1).N./(3*58*Rsurf) ]')


fprintf(1,'\n\napparent reflectance, uncorrected, strong\n');
fprintf(1,'\t%2.2f', Rsurf(1,:));
fprintf(1,'\n');
thestr=repmat('\t%2.2f', 1,size(C(1).med,2));
fprintf(1, ['%2.1f', thestr,'\n'], [Rough(:,1), U(2).N./(12*58*Rsurf)]')

fprintf(1,'\n\napparent reflectance, corrected, strong\n');
fprintf(1,'\t%2.2f', Rsurf(1,:));
fprintf(1,'\n');
thestr=repmat('\t%2.2f', 1,size(C(1).med,2));
fprintf(1, ['%2.1f', thestr,'\n'], [Rough(:,1), C(2).N./(12*58*Rsurf) ]')
