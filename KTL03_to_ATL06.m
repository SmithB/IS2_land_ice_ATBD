...
if ~isfield(D2,'x_RGT')
    for kB=1:2 
        D2(kB).x_RGT=D2(kB).dist_seg+D2(kB).dist_ph;
    end
end
...
for k0=1:length(seg_list)
    ...
    for kB=1:2
        ...
        if D3(k0, kB).signal_selection_source ==2 &&  D3(k0, kB).signal_selection_status_backup ~=3  % not enough confident or padded PE have been found, fall back to alternate strategies
            ...
            D2sub(kB)=index_struct(D2sub_unfilt(kB), initial_fit_els);
            initial_fit_els=true(size(D2sub(kB).h_ph));
...
%---------------------------------------------------------------------------------------
function [D3, r0, els, LOG]=ATLAS_LS_fit(D2, L0, initial_els,  N_it, params, D3, options, CONSTANTS)
...
% plotting command:
%clf; plot(real(D2.x0), D2.h_ph,'.'); hold on; plot(real(D2.x0(els)), D2.h_ph(els),'ro'), plot(real(D2.x0(els)), G(els,:)*m,'g')
...
selected_PE=abs(D2_local.h_ph-z0)<W/2;
