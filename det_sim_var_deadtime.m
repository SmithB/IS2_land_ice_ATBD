function D2=det_sim_var_deadtime(DEM, x0, params, ATM_xmit, CCR)
...
% NNote also: t_dead must be 1xN_det
...
for k=1:length(u_DS)
    ...
    if calc_gain
        ...
        dead(t_WF>ti(1) & t_WF < ti(1)+t_dead(channel)) = true;
    end
    %disp(k)
    while ~(hit(end) || missed(end))
        first=next_first;
        hit(first)=true;
        in_dt=ti>ti(first) & ti < ti(first)+t_dead(channel);
        if any(in_dt)
            % extend the dead time if there are photon hits while the counter is dead
            count=0;
            while sum(in_dt) > count
                count=sum(in_dt);
                in_dt=ti > ti(first) & ti < max(ti(in_dt))+t_dead(channel);
            end
            missed(in_dt) = true;
            last=find(in_dt, 1, 'last');
            if calc_gain
                dead(t_WF>ti(first) & t_WF < ti(last)+t_dead(channel)) = true;
            end
            next_first=last+1;
        else
            if calc_gain
                dead(t_WF>ti(first) & t_WF < ti(first)+t_dead(channel)) = true;
            end
            next_first=first+1;
        end
    end
