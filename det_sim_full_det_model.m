function D2=det_sim_full_det_model(DEM, x0, params, ATM_xmit)
...
[~, ind]=sort(((D.t_ph-min(D.t_ph))+1e-9)/1e-6 + D.pulse_num);
D=index_struct(D, ind);

D.BGR=zeros(size(D.x0))+params.NoiseRate;

[D.detected, D.channel] = deadtime2(D.t_ph, D.pulse_num,  params.N_channels, params.t_dead_analog, params.t_dead_digital);
