function D2a=read_ATLAS_h5_D2a(h5_file, pair, skip_sig)
...
top_groups={sprintf('/gt%dl', pair),  sprintf('/gt%dr', pair)};
...
if ~skip_sig
    for kG=1:length(top_groups)
        sig_group=['/channel', top_groups{kG}(2:end)];
        D2a(kG).ph_class=zeros(size(D2a(kG).h));
        try
            temp=h5read([this_dir,'/',sig_file.name],[sig_group,'/photon/ph_class']);
