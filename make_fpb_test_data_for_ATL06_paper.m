...
[Rough, Rsurf, U, C]=make_fpb_test_data_for_ATL06_paper_full_det_model('report_errors', thedir);
figure(51); clf;
 
cmap=my_rgb_cpt(128)*.8;
cmap=cmap([1:2:64, 65:128], :);
colormap(cmap)
set(gcf, 'units','inches','position', [12 3 7 5.2], 'papersize', [7.5 5.5], 'defaultaxesfontsize', 14,'color','w');
...
hb=colorbar('south'); set(hb,'color', 'w'); ylabel(hb,'bias, mm');
...
axes(hax(1,3));
imagesc(Rsurf(1,:), Rough(:,1), 100*U(2).N/(57*12)./(repmat(.125:.125:1, size(Rough,1), 1))-100); caxis([0.65 1.05]*100-100);
colormap(gca,'gray');
title('reflectance');
axes(hax(2,3));
imagesc(Rsurf(1,:), Rough(:,1), 100*C(2).N/(57*12)./(repmat(.125:.125:1, size(Rough,1), 1))-100); caxis([0.65 1.05]*100-100);
colormap(gca,'gray');
hb=colorbar('south');
ylabel(hb,'reflectance error, %');
set(hax(:,2),'ytick',[]);
